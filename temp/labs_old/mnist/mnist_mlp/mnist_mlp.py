...
参数说明:
    --datapath     字符串,数据保存路径,默认"~/data/mnist_mlp"
                   存放数据库文件 ~/data/mnist_mlp/state.db
                   存放训练模型文件 ~/data/mnist_mlp/*.yaml
                   存放训练结果文件 ~/data/mnist_mlp/*.dat
    --fun          脚本功能,默认0
                   0-超参组合批量训练
                   1-查询最佳训练模型
                   2-模型预测
    ...
    {模型预测}
    --modelfile    模型参数文件,默认""
                   模型文件命名规则(文件名不能含逗号):"hidlayer_num-hidlayer_nerouns-dropout-activation",eg."2-(300_500)-0.2-linear"
                     hidlayer_num ------- 隐层层数
...
使用范例:
    #批量超参模型训练
    $python3 mnist_mlp.py --fun 0 --datapath ~/data/mnist_mlp --max_trains 3000 --max_tests 300 --batch_size 128 --epochs 10 --hidlayers "2,3" --neurons "400,500,600" --dropouts "0.2,0.4" --activations
 "linear,relu,tanh"
    $python3 mnist_mlp.py --fun 0 --datapath ~/data/mnist_mlp --batch_size 128 --epochs 20 --hidlayers "2,3,4" --neurons "256,400,512,600,700" --dropouts "0.2,0.4" --activations "linear,relu,tanh"
...
class Data:
    ...
    def Add(self,model_file,model,history,score,y_predict,y_test,samples):
        ''' 添加数据库记录
        @param model_file:模型文件,不含扩展名,含参数表达 "hidlayer_num-hidlayer_nerouns-dropout-activation",eg."2-(300_500)-0.2-linear"
        @param model: 模型
        @param history:训练状态
        ...
        @param samples:训练样本数
        '''
        str_args=model_file.split('-')                     #分离参数[hidlayer_num,hidlayer_nerouns,dropout,activation]
        hidlayer_num=str_args[0]                           #隐层数目
        hidlayer_nerouns=str_args[1]                       #隐层神经元数目
        dropout=str_args[2]                                #dropout系数
        activation=str_args[3]                             #激活函数
        test_num=y_test.shape[0]                           #测试数目
        ...
        sql="insert into data(model_file,hidlayer_num,hidlayer_nerouns,dropout,activatieon,samples,tests"\
                         ...
                         ", %f,%f, %f,%f, %f)"\
                         %(model_file.replace(' ',''),hidlayer_num,hidlayer_nerouns,dropout,activation,samples,test_num
                         ,train_loss,train_acc,valid_loss,valid_acc,test_acc)
    ...
    def GetBestModel(self):
        ...
        self.conn.commit()
        row=cur.fetchone()
        return (row[0],row[1],row[2],row[3]) #[model_file,train_acc,valid_acc,test_acc]
'''class Data[End]''''''''''''''''''''''''''''''''''''''''''''''''
'''
       ...
       for v2 in neurons[v1]:
            for v3 in dropouts: 
                for v4 in activations:
                    filepath='%s/%s-%s-%s-%s.yaml' %(to_path,v1,str(v2).replace(' ','').replace(',','_'),v3,v4)
...
#-------------------------------
def train(model,x_train,y_train,validation_split=0.2,batch_size=128,epochs=10,log_dir=''):
    ''' 训练
    @param model:网络模型
...
def coms_train(arg):
    ...
    hidnerouns={}
    for i in hidlayers:
        hidnerouns[i]=list(itertools.permutations(nerouns,i))  #排列生成隐层神经元数目
        ...
        if len(vals)==2 and vals[1]=='.yaml':
            ...
            if not db.IsExist(vals[0]):
                with open(sfile,'r') as f:
                    ...
                    log_dir='%s/log_dir/%s'%(arg.datapath,vals[0])#日志目录
                    #创建目录
                    if not os.path.exists(to_path):
                        os.makedirs(to_path)
                    history=train(model,x_train,y_train,0.2,arg.batch_size,arg.epochs,log_dir=log_dir)  #训练
                    score=evaluate(model,x_test,y_test)           #评估
                    y=predict(model,x_test)                       #测试
                    model.save(model_h5file)                      #保存模型参数
                    #添加记录
                    examples=y_train.shape[0]
                    db.Add(vals[0],model,history,score,y,y_test,examples)
                    del model
                    gc.collect()
...
def get_best_model(arg):
    ...
    db=Data(db_path)
    #查询
    best_rd=db.GetBestModel() #[model_file,train_acc,valid_acc,test_acc]
    model_file='%s.h5' %(best_rd[0])
    print('model_file:%s/model/%s'%(arg.datapath,model_file))
    print('log_dir:%s/log_dir/%s'%(arg.datapath,best_rd[0]))
...
def params():
    ...
    parser.add_argument('--datapath', type=str, help='数据目录. eg. --datapath "~/data/mnist_mlp"', default='%s/data/mnist_mlp'%(os.getenv('HOME')));
