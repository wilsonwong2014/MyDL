...
#   2.无参数选项可并在一起，如 -cd
#   3.无参数短选项必须放在最后
#   4.非选项参数不能混在选项中间，如 -a 1 2 -b 3 -c -d
demo_getopts()
{
  echo "----demo_getopts----"
  while getopts "a:b:cd" arg #选项后面的冒号表示该选项需要参数
  do
        case $arg in
             a)
                echo "a's arg:$OPTARG" #参数存在$OPTARG中
...
#测试
echo "----Toplevel----"
echo "demo1: sh demo_args.sh 1 2 3 4 5"
echo "demo2: sh demo_args.sh -a 1 -b 2 -c "
echo "demo3: sh demo_args.sh -a 1 -b 3 -c -d"
echo "demo4: sh demo_args.sh -a 1 -b 3 -cd"
echo "shell            \$0:$0"
echo "First arg        \$1:$1"
echo "Arg num          \$#:$#"
echo "Arg self         \$@:$@"
echo "Arg self(string) \$*:$*"
